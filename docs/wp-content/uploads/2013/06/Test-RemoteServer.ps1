...
Function Test-RemoteServer
{
    ...
    .PARAMETER TrueFalseMode
        By default if only a single test is specified then the results returned are either true or false.
        If this parameter is set to false, then a single test still results in a psobject with properties for 
    ...
    .EXAMPLE
        "server1","localhost" | Test-RemoteServer -PingTest:$true | ft
     ...
     .EXAMPLE
        Test-RemoteServer Server1 -RDPTest $true -PingTest $true | select Name,RPC,RDP | ft
    ...
    .NOTES
        ...
        Author     : Zachary Loeber
        Website    : http://zacharyloeber.com
    ...
    param(
        ...
        [bool]$RemoteRegistryTest=$false,
        [parameter( HelpMessage="Perform a WSMan test")]
        [bool]$WSManTest=$false,
    ...
    BEGIN
    {
        ...
        $TestCount = ([int]$PingTest + 
                      [int]$RPCTest + 
                      [int]$RDPTest + 
                      [int]$RemoteRegistryTest + 
                      [int]$WSManTest +
                      [int]$SCCMTest +
                      [int]$SCOMTest)
        $ReturnBooleansOnly = (($TestCount -eq 1) -and $TrueFalseMode)
    }
    PROCESS
    {
        $computernames = @()
